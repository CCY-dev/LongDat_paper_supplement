...
setwd("/Users/Jessica/Documents/Lab/SparseDossa2/Negative_control_forLongdat_correct/Simulated_data/Raw/")

# The following is for simulating longitudinal data (negative control)
...
n_t <- 2 # 2 time points
n_s <- 150 # number of subjects
effect_size = 0 #Negative control
...
for (i in 1:100) {
  ...
  colnames(simulated_df_t)[3:(ncol(simulated_df_t))] <- paste0("NoDiffBug_", 1:(ncol(simulated_df_t) - 2))

  # Change the ID names so that they are characters
  simulated_df_t$ID <- paste0("s_", simulated_df_t$ID)
  

  write.table(simulated_df_t, paste0("/Users/Jessica/Documents/Lab/SparseDossa2/Negative_control_forLongdat_correct/Simulated_data/Raw/Sparsedossa2_correct_NegCtrl_set", i, ".txt"), sep = "\t",
